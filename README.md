...
- **10.05.2022**
  - Усі підсистеми переведені на асинхрону архітектуру роботи з мережею для забезпечення максимальної ефективності та мінімального навантаження на системні ресурси.
  - Скрипт тепер не створює системний тредів, а параметр `-t` означає "максимальну кількість одначасних операцій з мережею". З новою архітектурою, скрипт повинен ефективно працювати зі значно більними значеннями параметру (до 10-12k).
  - Завдяки відмові від використання системних потоків, скрипт не призведе до "підвисання" усієї системи навіть при великих навантаженнях.
  - Навантаження між цілями динамічно перерозподіляється з метою депріорітезації цілей та атак які не працюють (або працюють неефективно).
  - Додано атаки `RHEX` та `STOMP`.
